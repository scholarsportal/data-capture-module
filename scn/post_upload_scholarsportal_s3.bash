...
do
	...
	if (( ( $err != 0 ) || ( $nl == 0 ) )); then
		...
		if [ ! `aws --endpoint-url https://s3.scholarsportal.info s3 ls s3://${S3HOLD}/${ulidFromJson}/`]; then    #this check is different than normal post_upload, we don't use the extra folder level
			...
			err=$?
			if (( $err != 0 )) ; then
				echo "dcm Error: zip package $ulid" 
				break
			fi

			shasum ${packageName}.${packageExt} > ${packageName}.sha

			aws --endpoint-url https://s3.scholarsportal.info s3 cp ${packageName}.${packageExt} s3://${S3HOLD}/${ulidFromJson}/ 
			err=$?
			if (( $err != 0 )) ; then
				echo "dcm Error while aws moving file $ulid : ${packageName}.${packageExt}" 
				break
			fi
			aws --endpoint-url https://s3.scholarsportal.info s3 cp ${packageName}.sha s3://${S3HOLD}/${ulidFromJson}/
			err1=$?
			if (( $err1 != 0 )) ; then
				echo "dcm Error while aws moving file $ulid : ${packageName}.sha" 
				break
			fi
			...
			sz=`aws --endpoint-url https://s3.scholarsportal.info s3 ls --summarize --human-readable s3://${S3HOLD}/${ulidFromJson}/${packageName}.${packageExt} | grep "Total Size: " | cut -d' ' -f 6`
